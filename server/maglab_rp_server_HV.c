...
bool write_to_binary_file(const char *filename);
bool read_binary_File(const char *filename);
		...
		*(uint32_t *)(cfg + 52) = pre;
		...
		{ 
		  ...
		  switch(command & 0x3) {
			...
			if(verbose)printf("writing to calibration file\n"); write_to_binary_file("/opt/bin/calibration_HV.dat");	
...
int read_calibration()
{
	if (read_binary_File("/opt/bin/calibration_HV.dat")){
		return 0;
	}
...
bool write_to_binary_file(const char *filename) {
	...
	if (fptr == NULL){
		perror("No File Exsists");
		return false;
	}
	size_t written = fwrite(&calibration, sizeof(rp_calib_params_t), 1, fptr);
	fclose(fptr);
    return (written == 1);
...
bool read_binary_File(const char *filename) {
    ...
    size_t itemsRead = fread(&calibration, sizeof(rp_calib_params_t), 1, fptr);
    fclose(fptr);
    return (itemsRead = 1);
}